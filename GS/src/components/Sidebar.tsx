...
export const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen, onCloseMobile }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Body */}
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 flex flex-col justify-between transition-transform duration-300 ease-in-out md:translate-x-0 ${
      ...
      >
        ...
        <div>
          <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="font-extrabold text-sm tracking-tight text-slate-900 dark:text-white leading-tight uppercase font-mono">
                  FEX Pharmacy
                </h2>
            ...
            <button
              onClick={onCloseMobile}
              className="md:hidden p-1 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors border border-slate-200 dark:border-slate-800"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="p-3 space-y-5">
            <div>
              <p className="px-2 text-[11px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2">
                Navegación
              </p>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg font-medium text-xs transition-all duration-150 ${
                        isActive
                          ? 'bg-indigo-50 dark:bg-slate-800 text-indigo-700 dark:text-white font-semibold border border-indigo-200 dark:border-slate-700 shadow-sm'
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className={isActive ? 'font-bold text-indigo-900 dark:text-white' : ''}>{item.label}</span>
                      </div>
                      {item.badge !== undefined && (
                        <span className={`px-1.5 py-0.5 text-[10px] font-mono font-bold rounded ${item.badgeColor || (isActive ? 'bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700/50')}`}>
                          {item.badge}
                        </span>
                      )}
                    </button>
        ...
        <div className="p-3 border-t border-slate-200 dark:border-slate-800 space-y-2.5 bg-slate-50 dark:bg-slate-950/40">
          ...
          <div className="bg-slate-100 dark:bg-slate-900 rounded-lg p-2.5 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 mb-1">
              <span className="flex items-center gap-1 font-bold text-slate-700 dark:text-slate-300">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" /> PWA v2.1
              </span>
              <span className="text-emerald-600 dark:text-emerald-400 font-mono font-bold">{activeBranchCount} ACT</span>
            </div>
