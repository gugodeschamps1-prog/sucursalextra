  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`fixed bottom-20 md:bottom-6 right-4 z-[9999] max-w-sm w-full bg-slate-900 text-slate-100 rounded-xl shadow-2xl border border-slate-800 border-l-4 ${getBorderColor()} p-3.5 font-mono`}
      >
        <div className="flex items-start gap-3">
          {getIcon()}
          <div className="flex-1 min-w-0">
            <h4 className="font-bold text-xs uppercase tracking-wider text-white">{toast.title}</h4>
            <p className="text-[11px] text-slate-400 mt-0.5 leading-relaxed line-clamp-3">
              {toast.body}
            </p>
          </div>
          <button
