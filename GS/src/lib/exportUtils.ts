...
export function generatePDFReport(options: ExportDataOptions) {
  const { branches, zones, provinces, customColumns, columnSettings, columnOrder, selectedMonth } = options;
  ...
  doc.text('FEX Pharmacy - Reporte General de Sucursales' + monthTitle, 14, 14);
  ...
  const fileName = selectedMonth 
    ? `Reporte_Sucursales_FEX_${selectedMonth}.pdf`
    : `Reporte_Sucursales_FEX_${new Date().toISOString().slice(0, 10)}.pdf`;
...
export function generateExcelReport(options: ExportDataOptions) {
  ...
  const fileName = selectedMonth 
    ? `Reporte_Sucursales_FEX_${selectedMonth}.xlsx`
    : `Reporte_Sucursales_FEX_${new Date().toISOString().slice(0, 10)}.xlsx`;
